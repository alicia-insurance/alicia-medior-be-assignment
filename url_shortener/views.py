...
def generate_unique_short_code(custom_alias=None):
    """
    Generates a unique short code. If a custom alias is provided, it checks if it is available; otherwise,
    it generates a random short code.

    Args:
        custom_alias (str, optional): The custom alias provided by the user. Defaults to None.
    ...
    if custom_alias:
        ...
        if ShortenedURL.objects.filter(short_code=custom_alias).exists():
            raise ValueError("This custom alias is already taken. Please choose another one.")
        return custom_alias
...
def shorten_url(request):
    """
    API view to shorten a given original URL. Optionally, a custom alias can be provided. 
    If a custom alias is not provided, a random one is generated. If the original URL already exists, 
    the existing shortened URL is returned.

    Args:
        request (Request): The HTTP request containing the original URL and optionally a custom alias.

    Returns:
        Response: A response containing the shortened URL or an error message.
    """
    original_url = request.data.get('original_url')
    custom_alias = request.data.get('custom_alias', None)  # Get the custom alias if provided

    if not original_url:
        return Response({"error": "original_url field is required"}, status=status.HTTP_400_BAD_REQUEST)

    if not custom_alias:
        # Now check if the original URL already exists in the database
        existing_mapping = ShortenedURL.objects.filter(original_url=original_url).first()
        
        if existing_mapping:
            # Return the existing short URL if the URL already exists in the database
            return Response(ShortenedURLSerializer(existing_mapping).data, status=status.HTTP_200_OK)
    ...
    shortened_url = ShortenedURL(original_url=original_url, short_code=short_code, custom_alias = custom_alias)
    shortened_url.save()

    # Return the short URL in the response
    return Response(ShortenedURLSerializer(shortened_url).data, status=status.HTTP_201_CREATED)
...
def get_url_stats(request, short_code):
    ...
    Returns:
        Response: A response containing the statistics (short code, original URL, access count).
    """
    shortened_url = get_object_or_404(ShortenedURL, short_code=short_code)
