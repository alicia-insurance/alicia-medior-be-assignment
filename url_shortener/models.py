...
class ShortURL(models.Model):
    original_url = models.URLField(
        null=False, blank=False, help_text="The original URL to be shortened."
    )
    ...
    access_count = models.PositiveIntegerField(
        default=0, help_text="Number of times the short URL has been accessed."
    )
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="Timestamp when the short URL was created."
    )
    is_active = models.BooleanField(
        default=True, help_text="Indicates if the short URL is active or not."
    )
    ...
    def generate_short_alias(cls, length=8, version='v1'):
        ...
        if version not in strategies:
            raise NotImplementedError(
                f"Short Code generation for version '{version}' is not implemented."
            )
